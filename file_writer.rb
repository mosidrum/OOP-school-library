class FileWriter
  def write_book_to_file(book)
    File.open('books.json', 'a') do |_file|
  ...
  def write_person_to_file(person)
    File.open('people.json', 'a') do |_file|
      data = {
        'Id' => person.id,
        'Name' => person.name,
        'Age' => person.age,
  ...
  def write_rental_to_file(rental)
    File.open('rental.json', 'a') do |_file|
      data = { 'Person' => rental.person.name, 'Book' => rental.book.title, 'Author' => rental.book.author,
               'Date' => rental.date }
